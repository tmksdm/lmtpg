...
def main():
    ...
    with sync_playwright() as p:
        ...
        # --- Шаг 1: открываем дашборд ---
        print(f"Открываю дашборд: {config.DASHBOARD_URL}")
        page.goto(config.DASHBOARD_URL)
        ...
        WAIT_FOR_VIDEO_MS = 20 * 60 * 1000   # 20 минут (на Этапе 5 вынесем в config)

        busy = loc(page, "busy_indicator")   # подсказка "заявка в работе"

        # 8.1 — ждём ПОЯВЛЕНИЯ индикатора (генерация началась).
        # Даём ему до 30 секунд, чтобы успеть прорисоваться после клика.
        print("Жду подтверждения, что генерация началась...")
        busy.wait_for(state="visible", timeout=30000)
        print("✅ Генерация началась (появилась подсказка 'заявка в работе').")

        # 8.2 — ждём ИСЧЕЗНОВЕНИЯ индикатора (генерация закончилась).
        # Вот тут уходит основное время — 5-15 минут. Лимит — 20 минут.
        print("Жду окончания генерации (5-15 минут)...")
        busy.wait_for(state="hidden", timeout=WAIT_FOR_VIDEO_MS)
        print("✅ Генерация завершена (подсказка пропала).")
